...
public class StreamerPlayer {
    ...
    public StreamerPlayer(@NotNull String _streamerPlayerName, DonationAlertsToken donationAlertsToken, DonationExecutor donationExecutor) {
        streamerPlayerName = _streamerPlayerName;
        Bukkit.getPluginManager().registerEvents(new PlayerDeathListener(this), donationExecutor);


        //Заполняем список сумм для донатов
        String amount;
        for (String execName : Executor.executionsNamesList) {
            amount = donationExecutor.getMainConfig().getAmounts().get(donationAlertsToken.getToken()).get(streamerPlayerName).get(execName);
            if (!(amount==null)) {
